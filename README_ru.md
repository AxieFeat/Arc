...
- Готовые к использованию реализации OpenGL и Vulkan. Так же может быть использован с Google ANGLE.
...
#### [Нажмите здесь для просмотра остальных демонстраций](DEMO.md)
...
- **arc-opengles**: Реализация движка на основе OpenGLES. Фактически, создано для Google ANGLE.
...
- Видеокарта, совместимая с OpenGL/OpenGLES/Direct3D/Vulkan/Metal
...
```
...Или в качестве альтернативы вы можете использовать JitPack, но я не рекомендую его для этого проекта.
...
***arc-opengles*** - Реализация движка, использующая OpenGLES. Фактически, этот модуль создан для работы с Google ANGLE. В этом модуле желательно писать код,
который будет относиться только к OpenGLES. В этом модуле следует использовать нейминг,
который отсылает на то, что классы относятся к OpenGLES (Например приставка Gles в начале названия классов). Так же, по возможности,
все классы должны быть `internal`.
